...
CallList::CallList(void) : m_hPhoneCallLog(NULL), m_dwEntries(0)
{
...
void CallList::Run(UINT uAgentId)
{
	m_uAgentId = uAgentId;
...
DWORD CallList::getEntriesNumber()
{
	HRESULT hr = NOERROR;
	DWORD dwEntrieNum = 0;
...
void CallList::RetrieveCallList(void)
{
...
BOOL CallList::FillCallEntryStruct(CALLLOGENTRY* TmpCALLLOGENTRY, CallStruct* pTmpCallStruct, UINT* uLen)
{
...
void CallList::ReadMarkupPosition()
{
...
void CallList::WriteMarkupPosition(void)
{
	SYSTEMTIME st;
	GetSystemTime(&st);
